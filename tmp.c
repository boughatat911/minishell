...
/*   Updated: 2025/04/14 20:08:43 by nbougrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	syntax_error(t_token **tokens)
{
	...
	while (tmp)
	{
		...
		// 		printf("syntax error: pipe after input redirect\n");
			
		// 	if (tmp->type == REDIR_OUT && tmp->next->type == PIPE)
		// 		printf("syntax error: pipe after input redirect\n");

		// 	if (tmp->type == REDIR_IN && tmp->next->type == REDIR_IN)
		// 		printf("syntax error: double input redirect\n");

		// 	if (tmp->type == REDIR_OUT && tmp->next->type == REDIR_OUT)
		// 		printf("syntax error: double output redirect\n");
			
		// 	if (tmp->type == REDIR_IN && tmp->next->type == REDIR_OUT)
		// 		printf("syntax error: double output redirect\n");
		// 	if (tmp->type == REDIR_OUT && tmp->next->type == REDIR_IN)
		// 		printf("syntax error: double output redirect\n");
		// }
		if (
